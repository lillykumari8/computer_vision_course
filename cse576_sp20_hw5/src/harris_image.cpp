...
Image make_1d_gaussian(float sigma)
  ...
  for (int i=0; i<size; i++) {
    int x = i - offset;
    float pixel_val = (1/sqrtf(2*M_PI*sigma*sigma))*exp((-x*x)/(2*sigma*sigma));
    lin.set_pixel(i, 0, 0, pixel_val);
  }
  lin.l1_normalize();
  return lin;
  }
...
Image smooth_image(const Image& im, float sigma)
  ...
  Image first_conv = convolve_image(im, filter, true);
  swap(filter.h, filter.w);
  Image sec_conv = convolve_image(first_conv, filter, true);
  return sec_conv;
  }
...
Image structure_matrix(const Image& im2, float sigma)
  ...
  for (int i=0; i<im.w; i++) {
    for (int j=0; j<im.h; j++) {
      float pixel_ix = Ix.clamped_pixel(i, j, 0);
      float pixel_iy = Iy.clamped_pixel(i, j, 0);
      S.set_pixel(i, j, 0, pixel_ix*pixel_ix);
      S.set_pixel(i, j, 1, pixel_iy*pixel_iy);
      S.set_pixel(i, j, 2, pixel_ix*pixel_iy);
    }
  }
...
Image cornerness_response(const Image& S, int method)
  ...
  if (method==0) {
    for (int i=0; i<S.w; i++) {
      for (int j=0; j<S.h; j++) {
        float IxIx = S.clamped_pixel(i, j, 0);
        float IyIy = S.clamped_pixel(i, j, 1);
        float IxIy = S.clamped_pixel(i, j, 2);

        float det_S = IxIx*IyIy -IxIy*IxIy;
        float trace_S = IxIx + IyIy;
        float deno = max_float(-1e10f, trace_S);
        float val = det_S / (deno);
        R.set_pixel(i, j, 0, val);
      }
    }
  }
  else{
    printf("choosing eigen value \n");
    for (int i=0; i<S.w; i++) {
      for (int j=0; j<S.h; j++) {
        float IxIx = S.clamped_pixel(i, j, 0);
        float IyIy = S.clamped_pixel(i, j, 1);
        ...
        float second_part = sqrtf(4*IxIy*IxIy + powf((IxIx - IyIy), 2.0));
        float first_part = IxIx + IyIy;
        float lambda_1 = 0.5 * (first_part + second_part);
        float lambda_2 = 0.5 * (first_part - second_part);
        float lambda = min_float(lambda_1, lambda_2);
        R.set_pixel(i, j, 0, lambda);
      }
    }    
  }
  return R;
...
Image nms_image(const Image& im, int w)
  ...
  for (int i=0; i<im.w; i++) {
    for (int j=0; j<im.h; j++) {
      float pixel_res = im.clamped_pixel(i, j, 0);
      for (int x=i-w; x<=i+w; x++) {
        for (int y=j-w; y<=j+w; y++) {
          float nb_res = im.clamped_pixel(x, y, 0);
          if (nb_res > pixel_res) {
            r.set_pixel(i, j, 0, -1e10f);
            goto here;
          }
        }
      }
...
vector<Descriptor> detect_corners(const Image& im, const Image& nms, float thresh, int window)
  ...
  for (int i=0; i<nms.w; i++) {
    for (int j=0; j<nms.h; j++) {
      float pixel_val = nms.clamped_pixel(i, j, 0);
      if (pixel_val > thresh) count_thresh += 1;
    }
  }
  ...
  for (int i=0; i<nms.w; i++) {
    for (int j=0; j<nms.h; j++) {
      if (nms.clamped_pixel(i, j, 0) > thresh) {
