...
void do_columbia_peak(void)
  ...
  th.push_back(thread([&](){create_panorama(im,"0-1","0","1" ,2,0,0.05,7,7,5,50000,100,0.9);}));
  ...
  th.push_back(thread([&](){create_panorama(im,"0--3" ,  "0-1",  "2-3" ,2,0,0.04,11,7,5,50000,100,0.7);}));
  th.push_back(thread([&](){create_panorama(im,"4--7" ,  "4-5",  "6-7" ,2,0,0.04,11,7,5,50000,100,0.9);}));
  for(auto&e1:th)e1.join();th.clear();
...
void do_field(void)
  ...
  image_map im;
  load_images(im,indir,outdir,8,1,1150);  // im, dir, numpics (0..numpics-1),  PROJ_METHOD,  FOCAL_LEN
  
  vector < thread > th; 
  
  
  th.push_back(thread([&](){create_panorama(im,"2-3","2","3" ,3,0,0.05,11,7,5,50000,100,1);}));
  ...
  th.push_back(thread([&](){create_panorama(im,"4--7", "4-5",  "6-7" ,3,0,0.05,11,7,5,50000,100,1.0);}));
  for(auto&e1:th)e1.join();th.clear();
...
void do_helens(void)
  ...
  image_map im;
  load_images(im,indir,outdir,6,1,1000);  // im, dir, numpics (0..numpics-1),  PROJ_METHOD,  FOCAL_LEN
  
  vector < thread > th; 
  
  
  th.push_back(thread([&](){create_panorama(im,"0-1","0","1" ,2,0,0.05,11,7,5,150000,100,0.8);}));
  th.push_back(thread([&](){create_panorama(im,"2-3","2","3" ,2,0,0.05,11,7,5,150000,100,0.3);}));
  th.push_back(thread([&](){create_panorama(im,"4-5","4","5" ,2,0,0.05,11,7,5,150000,100,0.6);}));
  for(auto&e1:th)e1.join();th.clear();
  
  
  create_panorama(im,"0--3", "0-1",  "2-3" ,2,0,0.05,11,7,5,50000,100,0.8);
  create_panorama(im,"all" , "0--3", "4-5" ,2,0,0.05,11,7,5,50000,100,0.8);
...
void do_sun(void)
  ...
  th.push_back(thread([&](){create_panorama(im,"0-1","0","1" ,2,0,0.04,11,7,10,150000,100,0.9);}));
  th.push_back(thread([&](){create_panorama(im,"2-3","2","3" ,2,0,0.04,11,7,10,150000,100,0.2);}));
  for(auto&e1:th)e1.join();th.clear();
  
  create_panorama(im,"2--4", "2-3",  "4"   ,2,0,0.05,11,7,10,50000,100,0.1);
  create_panorama(im,"all" , "0-1", "2--4" ,2,0,0.05,11,7,10,50000,100,0.1);
...
void do_my(void)
  {
  string indir="pano/y24/";
  string outdir="output/y24/";
 
  image_map im;
  load_images(im,indir,outdir,8,1,1000);  // im, dir, numpics (0..numpics-1),  PROJ_METHOD,  FOCAL_LEN
 
  vector < thread > th;
 
 
  th.push_back(thread([&](){create_panorama(im,"0-1","0","1" ,2,0,0.1,11,7,5,150000,100,0.1);}));
  th.push_back(thread([&](){create_panorama(im,"2-3","2","3" ,2,0,0.1,11,7,5,150000,100,0.1);}));
  th.push_back(thread([&](){create_panorama(im,"4-5","4","5" ,2,0,0.02,11,7,5,150000,100,0.1);}));
  th.push_back(thread([&](){create_panorama(im,"6-7","6","7" ,2,0,0.02,11,7,5,150000,100,0.1);}));
  for(auto&e1:th)e1.join();th.clear();
 
 
  create_panorama(im,"0--3",  "2-3" , "0-1", 2,0,0.1,11,7,5,50000,100,0.0);
  create_panorama(im,"4--7" , "6-7" ,"4-5",  2,0,0.05,11,7,5,50000,100,0.9);
  create_panorama(im,"all"  ,  "4--7", "0--3", 2,0,0.05,11,7,5,50000,100,0.9);
  save_images(im,outdir);
