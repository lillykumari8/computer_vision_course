...
Matrix backward_w(const Layer &l) {
  ...
  grad_w = x.transpose() * l.grad_out1;
...
Matrix backward_x(const Layer &l) {
  ...
  grad_x = l.grad_out1 * w.transpose();
...
void update_layer(Layer &l, double rate, double momentum, double decay) {
  ...
  // NOT_IMPLEMENTED();
  Matrix neww(l.v.rows, l.v.cols);
  neww = l.grad_w -  (decay * l.w) + (momentum * l.v);
  l.v = neww;
  // TODO: update the weights and save to l.w.
  // Hint: w_{t+1} = w_t + ηΔw_t
  // NOT_IMPLEMENTED();
  Matrix weights(l.w.rows, l.w.cols);
  weights = l.w + (rate*l.v);
  l.w = weights;
}
...
double l1_loss(const Matrix &y, const Matrix &p) {
  ...
  for (int i = 0; i < y.rows; i++) {
    for (int j = 0; j < y.cols; j++) {
      if (fabs(y(i,j)-p(i,j)) > 1) sum += (fabs(y(i, j) - p(i, j)) - 0.5);
      else sum += 0.5 * pow(y(i, j) - p(i, j), 2);
    }
  }
  return sum / y.rows;  
  // NOT_IMPLEMENTED();
...
Matrix Model::loss_derivative(const Matrix &y, const Matrix &p)
  {
    ...
    for(int q1=0;q1<d.rows;q1++)for(int q2=0;q2<d.cols;q2++){
      if (fabs(y(q1,q2)-p(q1,q2)) > 1) d(q1,q2) = 2*(y(q1,q2)>p(q1,q2))-1;
      else d(q1,q2) = (y(q1,q2) - p(q1,q2));
    }
    return d;
    }
  else assert(false && "Invalid loss function");
